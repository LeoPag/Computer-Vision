...
def main():
  ...
  number_of_points = [None, None, None, None]
  for pose in range(4):
      e_im1.SetPose(possible_relative_poses[pose][0], possible_relative_poses[pose][1])
      points3D, im1_corrs, im2_corrs = TriangulatePoints(K, e_im1, e_im2, e_matches)
      number_of_points[pose] = len(points3D)

  correct_pose = np.argmax(number_of_points)

  # TODO
  # Set the image poses in the images (image.SetPose(...))
  e_im2.SetPose(np.eye(3), np.array([0, 0, 0]))
  e_im1.SetPose(possible_relative_poses[correct_pose][0], possible_relative_poses[correct_pose][1])
  ...
  #points3D = points3D[np.linalg.norm(points3D, axis = 1, ord = 2) < 10, :]
