...
class SegNetLite(nn.Module):
    ...
    def __init__(self, kernel_sizes=[3, 3, 3, 3], down_filter_sizes=[32, 64, 128, 256],
        ...
        # following output dimension (ignoring batch dimension):
        ...
        inputchannelsdown = [3,32,64,128]

        for i in range(self.num_down_layers):
            layers_conv_down.append(nn.Conv2d(inputchannelsdown[i],down_filter_sizes[i],kernel_sizes[i],padding = conv_paddings[i]))
        ...
        # following output dimension (igmoring batch dimension):
        ...
        for j in range(self.num_up_layers):
            layers_conv_up.append(nn.Conv2d(down_filter_sizes[self.num_up_layers - 1 - j],up_filter_sizes[j],kernel_sizes[j],padding = conv_paddings[j]))
            layers_bn_up.append(nn.BatchNorm2d(up_filter_sizes[j]))
            layers_unpooling.append(nn.MaxUnpool2d(pooling_kernel_sizes[j],pooling_strides[j]))
        #raise NotImplementedError('Upsampling layers are not implemented!')
        ...
        self.final = nn.Conv2d(32,11,1)
        #raise NotImplementedError('Final convolution layer is not implemented!')

    def forward(self, x):

        indeces = [None,None,None,None]

        for i in range(self.num_down_layers):
            x = self.layers_conv_down[i](x)
            x = self.layers_bn_down[i](x)
            x = self.relu(x)
            x,indeces[i] = self.layers_pooling[i](x)

        for j in range(self.num_up_layers):
            x = self.layers_unpooling[j](x, indeces[self.num_up_layers - 1 - j])
            x = self.layers_conv_up[j](x)
            x = self.layers_bn_up[j](x)
            x = self.relu(x)
